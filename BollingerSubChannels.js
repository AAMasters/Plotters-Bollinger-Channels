function newAAMastersPlottersBollingerChannelsBollingerSubChannels() {
            ...
            while (currentDate.valueOf() <= farRightDate.valueOf() + ONE_DAY_IN_MILISECONDS) {
                ...
                if (dailyFile !== undefined) {
                    ...
                    for (let i = 0; i < dailyFile.length; i++) {
                        ...
                        channel.firstDeviation = dailyFile[i][7];
                        channel.lastDeviation = dailyFile[i][8];
                        ...
                        if (channel.begin >= farLeftDate.valueOf() && channel.end <= farRightDate.valueOf()) {
                            ...
                            if (datetime.valueOf() >= channel.begin && datetime.valueOf() <= channel.end) {
                                ...
                                thisObject.container.eventHandler.raiseEvent("Current Sub-Channel Changed", thisObject.currentChannel);
    ...
    function recalculateUsingMarketFiles() {
        ...
        try {
            ...
            for (let i = 0; i < marketFile.length; i++) {
                ...
                channel.firstDeviation = marketFile[i][7];
                channel.lastDeviation = marketFile[i][8];
    ...
    function plotChart() {
        ...
        try {
            ...
            thisObject.container.eventHandler.raiseEvent("Current Sub-Channel Changed", currentChannel);
            ...
            if (channels.length > 0) {
                ...
                for (var i = 0; i < channels.length; i++) {
                    ...
                    if (channel.direction === 'Side' || channel.slope === 'Side') { browserCanvasContext.fillStyle = 'rgba(' + UI_COLOR.LIGHT + ', ' + opacity + ')'; }

                    switch (channel.slope) {
                        case 'Side': {
                            opacity = '0.1';
                            lineWidth = 0.25;
                            break;
